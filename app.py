...
# ---------------- BLYNK CONFIG ----------------
# V1 = order data string this app writes (fp, tp, F, ID)
# V3 = "reached" flag the server writes (1 = bot has arrived, 2 = intermediate waypoint hit)
# V4 = "box opened" confirmation this app writes after ID re-check passes
...
if not st.session_state.order_placed:
    ...
    if st.button("📦 Place Order"):
        if not user_id:
            ...
            # Format the data string exactly how the server expects it
            data_string = f"fp:{from_place},tp:{to_place},F:{food},ID:{user_id}"
            
            # CRITICAL FIX: Send data string to V1 (Server's incoming pipeline)
            status = send_to_pin("v1", data_string)
            if status == 200:
...
# ========================
    ...
    if not st.session_state.reached:
        ...
        # CRITICAL FIX: Read V3 to check the final arrival flag from server
        val = read_pin("v3")
        if val == "1":
            st.session_state.reached = True
            st.rerun()
        else:
            # Wait 4 seconds before pulling updates from Blynk again
            time.sleep(4)
            st.rerun()
        ...
        if not st.session_state.box_opened:
            ...
            if st.button("🔓 Confirm & Open Box"):
                if confirm_id and confirm_id == st.session_state.order_id:
                    # CRITICAL FIX: Send open command (1) to V4 instead of overriding V3
                    status = send_to_pin("v4", 1)
                    if status == 200:
            ...
            if st.button("🔁 Place New Order"):
                ...
                # Clear Blynk pins back to zero for the next run
                send_to_pin("v3", 0)
                send_to_pin("v4", 0)
                st.rerun()
